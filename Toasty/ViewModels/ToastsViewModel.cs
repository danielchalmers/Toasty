...
namespace Toasty.ViewModels
{
	public class ToastsViewModel : ViewModelBase
	{
		...
		public ToastsViewModel()
		{
			...
			GenerateTestToast = new RelayCommand(GenerateTestToastExecute);
		...
		{
			if (e is DirectoryScannerEventArgs args)
			{
				DirectoryWatcher_OnEvent(args);
			}
		}
		...
		{
			return Settings.Default.DockPosition switch
			{
				DockPosition.Right => SystemParameters.WorkArea.Right - ActualWidth,
				_ => 0,
			};
		}
